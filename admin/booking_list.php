...
<head>
  ...
  <link rel="stylesheet" href="_asset/boostrap/css/adminlte.min.css">
    <!-- Custom style -->
    <link rel="stylesheet" href="_asset/css/booking_list.css">
        ...
        <div class="container-fluid">
                ...
                <div class="card card-secondary">
                  <div class="card-header">
                    ...
                    <div class="court-filter">
                            <select id="court" name="court" onChange="switchCourt(this.value);">
                            <?php
                  ...
                  <!-- /.card-header -->
                    ...
                    <div class="card-footer">
                      <div class='form-input'>
                          <button class="btn btn-s btn-secondary " onclick="printDiv('schedule')"><i class="fas fa-print"></i> Print</button>
            ...
            <div class="col-12">
              <div class="card card-secondary collapsed-card">
                <div class="card-header">
                  <h3 class="card-title">Cancelled Booking</h3>
                ...
                <!-- /.card-header -->    
<?php 
    $cancelled_array = $db_handler->runQuery("SELECT reservation_id,reservation_edit,user_name,court_name,reservation_date,reservation_slot, reservation_remark, reservation_status
    FROM reservation JOIN user USING(user_id) JOIN court USING(court_id)
    WHERE reservation_status='Cancelled'
    ORDER BY reservation_id DESC;");
?>
                <div class="card-body table-responsive p-0" style="height: 300px;">
                  <table class="table table-head-fixed table-hover text-nowrap cancelled">
                    <thead>
                      <tr>
                        <th>#</th>
                    ...
                    <tbody>
<?php
  if (!empty($cancelled_array)) { 
  foreach($cancelled_array as $key=>$value){
?>
                      <tr>
                        <td><?php echo $cancelled_array[$key]["reservation_id"]; ?></td>
                        <td><?php echo $cancelled_array[$key]["reservation_edit"]; ?></td>
                        <td><?php echo $cancelled_array[$key]["user_name"]; ?></td>
                        <td><?php echo $cancelled_array[$key]["court_name"]; ?></td>
                        <td><?php echo $cancelled_array[$key]["reservation_date"]; ?></td>
                        <td><?php echo $cancelled_array[$key]["reservation_slot"]; ?></td>
                        <td><?php echo $cancelled_array[$key]["reservation_remark"]; ?></td>
                        <td><button type="button" class="btn" onClick="viewMore(<?php echo $cancelled_array[$key]['reservation_id']; ?>)" ><i class="fas fa-search-plus "></i></button></td>
                        <td><?php echo $cancelled_array[$key]["reservation_status"]; ?></td>
                      </tr>
<?php }} ?>
          ...
          <div class="row">
              <div class="col-12">
                ...
                <div class="card-tools">
                  ...
                  <!-- /.card-header -->
<?php 
    $complete_array = $db_handler->runQuery("SELECT reservation_id,reservation_edit,user_name,court_name,reservation_date,reservation_slot, reservation_remark, reservation_status
    FROM reservation JOIN user USING(user_id) JOIN court USING(court_id)
...
?>
                      <tr>
                        <td><?php echo $complete_array[$key]["reservation_id"]; ?></td>
                        <td><?php echo $complete_array[$key]["reservation_edit"]; ?></td>
                        <td><?php echo $complete_array[$key]["user_name"]; ?></td>
                        <td><?php echo $complete_array[$key]["court_name"]; ?></td>
                        <td><?php echo $complete_array[$key]["reservation_date"]; ?></td>
                        <td><?php echo $complete_array[$key]["reservation_slot"]; ?></td>
                        <td><?php echo $complete_array[$key]["reservation_remark"]; ?></td>
                        <td><button type="button" class="btn" onClick="viewMore(<?php echo $complete_array[$key]['reservation_id']; ?>)" ><i class="fas fa-search-plus "></i></button></td>
                        <td><button type="button" class="btn btn-primary btn-sm" onClick="editIt(<?php echo $complete_array[$key]['reservation_id']; ?>)" style='width:2.5em'><i class="fas fa-cog"></i></button></td>
                      </tr>
<?php }} ?>
                    </tbody>
                    </table>
...
<script type="text/JavaScript">
    ...
    function switchCourt(str){
      if(week==''){
        $("#schedule").load('schedule.php?CID='+str);
      }
      else{
        $("#schedule").load('schedule.php?CID='+str+'&week='+week+'&year='+year);
      }
      return;  
    }
    ...
    $( document ).ready(function() {
      if(cid==''){
        $("#schedule").load('schedule.php?CID=1');
      }
      else{
        $("#schedule").load('schedule.php?CID='+cid+'&week='+week+'&year='+year);
      }
      return;
    });
...
function printDiv(elem)
{
  ...
  document.querySelector('#'+ elem).style.transform = 'rotate(90deg)';
  document.querySelector('#'+ elem).style.height='900px';
  document.querySelector('#'+ elem).style.width='900px';
