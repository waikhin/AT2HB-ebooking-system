...
<head>
  ...
  <title>AT2HB | Announcement</title>
            ...
            <div class="row">
              <div class="col-12">
                ...
                <div class="card-tools">
                  ...
                  <!-- /.card-header -->
<?php 
    $announnce_array = $db_handler->runQuery("SELECT announcement_id,announcement_submit,announcement_detail
    FROM announcement 
    ORDER BY announcement_submit DESC;");
?>
                  <div class="card-body table-responsive p-0" style="height: 500px;">
                    <table class="table table-head-fixed table-hover text-nowrap">
                      <thead>
                        <tr>
                          <th>#</th>
                          <th>Announced Submitted</th>
                          <th>Announcement</th>
                          <th>Action</th>
                        </tr>
                      </thead>
                      <tbody><form id="announceForm">
<?php
  if (!empty($announnce_array)) { 
  foreach($announnce_array as $key=>$value){
?>
                        <tr>
                        <td><?php echo $announnce_array[$key]["announcement_id"]; ?></td>
                        <td><?php echo $announnce_array[$key]["announcement_submit"]; ?></td>
                        <td><?php echo $announnce_array[$key]["announcement_detail"]; ?></td>
                          <td><button type="button" class="btn btn-danger btn-sm" onclick="removeIt(<?php echo $announnce_array[$key]['announcement_id']; ?>)" style='width:2.5em'><i class="fas fa-times"></i></button></td>
                        </tr>
<?php }} ?>
                      </form>
                      </tbody>
...
function addNew(){
    $("#detail").load('_asset/process/announce_ctrl.php?action=announceForm');
    return;     
}

function removeIt(rid){    
    $.ajax({
        type: 'post',
        url: '_asset/process/announce_ctrl.php?action=remove',
        data: {
            rid:rid ,
        },
